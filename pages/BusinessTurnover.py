...
#         # Sort numerically by broker number
#         broker_options = broker_options.sort_values("number", ascending=True)

#         # Format as "12345 - Trishakti Securities"
#         broker_options_list = broker_options.apply(
#             lambda row: f"{row['number']} - {row['name']}", axis=1
#         ).tolist()
...
#         # KPIs
#         total_market_turnover = df["totalAmount"].sum()
#         trishakti_turnover = df.loc[
#             df["name"].str.strip().str.lower() == "trishakti securities public limited",
#             "totalAmount"
#         ].sum()
...
#                 st.metric(f"{selected_name} Market Contibution",f"{total_contribution:.4f} %", border=True)
#         else:
#             with col1:
#                 total_contribution = (trishakti_turnover/total_market_turnover)*100
#                 st.metric(f"Trishakti Market Contibution",f"{total_contribution:.4f} %", border=True)
...
#     def compare_data_with_last_fiscal_year(self, df):
#         # -------------------------------
#         # 1. Fiscal Year Selection
#         # -------------------------------
#         fiscal_years = ["81/82", "82/83"]
#         last_fy = fiscal_years[0]
#         this_fy = fiscal_years[1]

#         last_fy_start, last_fy_end = helper.get_fiscal_year_dates(last_fy)
#         this_fy_start, this_fy_end = helper.get_fiscal_year_dates(this_fy)
...
#             "Select Range",
#             ['1 Month', '3 Months', '6 Months', '8 Months', 'YTD'],
#             horizontal=True
#         )

#         # Compute start date based on selection
#         today = date.today()
...
#         elif date_range_option == '8 Months':
#             range_start = today - timedelta(days=240)
#         elif date_range_option == 'YTD':
#             # Year-to-date based on fiscal year
#             range_start = this_fy_start
#         else:
#             range_start = this_fy_start
...
#         # -------------------------------
#         # 3. Filter DF by Fiscal Year & Range
#         # -------------------------------
#         df["date"] = pd.to_datetime(df["date"])  # ensure date column is datetime

#         last_fy_df = df[
#             (df["date"] >= max(last_fy_start, range_start)) &
#             (df["date"] <= min(last_fy_end, range_end))
#         ]

#         this_fy_df = df[
#             (df["date"] >= max(this_fy_start, range_start)) &
#             (df["date"] <= min(this_fy_end, range_end))
#         ]

#         # -------------------------------
#         # 4. Aggregate Total Turnover
#         # -------------------------------
#         last_fy_total = last_fy_df["totalAmount"].sum()
#         this_fy_total = this_fy_df["totalAmount"].sum()
...
#         with col1:
#             st.metric(
#                 label=f"Fiscal Year {last_fy}",
#                 value=f"NPR {last_fy_total:,.2f}"
#             )
#         with col2:
#             st.metric(
#                 label=f"Fiscal Year {this_fy}",
#                 value=f"NPR {this_fy_total:,.2f}"
#             )
...
#         # -------------------------------
#         if last_fy_total > 0:
#             change_pct = ((this_fy_total - last_fy_total) / last_fy_total) * 100
#         else:
#             change_pct = 0

#         st.metric(
#             label="Change from Last FY (%)",
...
#         elif view == "Compare":
#             df = fetch_top_brokers_cached(self.start_date, self.end_date)
#             self.compare_data_with_last_fiscal_year(df)
#         else:
#             st.info("View not seleted", icon="📢")
#             st.stop()
