...
#                 "help": "Open client ledger dialog"
#             }
#         },
#         key="global"
#     )
...
#                 "From Date",
#                 value=date(2025, 7, 17),     # ✅ fixed start
#                 # min_value=date(2024, 1, 1), # 🔒 cannot go earlier
...
#                 "To Date",
#                 value=date.today(),         # ✅ till now
#                 min_value=from_date,        # 🔒 logical range
#                 max_value=date.today()
#             )
...
#                     st.error(f"Failed to fetch ledger: {e}")
#                     return
...
#             # c3.metric("A/C Name: ", f"{adjusted_balance:,.2f}")
...
#                 styled_df = (
#                     df.style
#                         .format(accounting_format, subset=number_cols)
#                     .map(highlight_negative, subset=number_cols)
#                 )

#                 # --- show in Streamlit ---
#                 st.dataframe(styled_df, width="stretch", height=300, hide_index=True)
...
#                 # coerce numeric for debit/credit/balance
...
#                 # style only those numeric columns
#                 styled_df = (
#                     df_ub.style
#                     .format(accounting_format, subset=num_cols)
#                     .map(highlight_negative, subset=num_cols)
#                 )
...
#     # Trigger dialog
#     if hotkeys.pressed("open_client_dialog"):
...
def activate_client_code_hotkey():
    ...
    if st.button("", shortcut="L", key="hidden_open_ledger"):
